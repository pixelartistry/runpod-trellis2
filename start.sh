...
# Target: RTX 5090 | Python 3.12 | PyTorch 2.9.1 | CUDA 12.8
# =============================================================================
...
# ─── Step 0: Ensure Python 3.12 ───────────────────────────────────────────────
ensure_python312() {
    step "Checking Python 3.12"
    if command -v python3.12 &>/dev/null; then
        log "Python 3.12 already available: $(python3.12 --version)"
        return 0
    fi

    warn "python3.12 not found — installing from deadsnakes PPA..."
    apt-get update -qq
...
# ─── Step 2: Install PyTorch 2.9.1 + CUDA 12.8 ───────────────────────────────
install_pytorch() {
    step "PyTorch 2.9.1 + CUDA 12.8"

    # Check if correct version is already installed
    if python3.12 -c "import torch; v=torch.__version__; assert v.startswith('2.9'), v" 2>/dev/null; then
        log "PyTorch 2.9.x already installed: $(python3.12 -c 'import torch; print(torch.__version__)')"
        return 0
    fi
    ...
    if ! python3.12 -m pip install \
        ...
        warn "Stable 2.9.1 not found — trying nightly cu128..."
        python3.12 -m pip install --pre \
...
# ─── Step 3: Install ComfyUI Python requirements ──────────────────────────────
install_comfyui_deps() {
    step "ComfyUI core dependencies"
    python3.12 -m pip install -r "$COMFYUI_DIR/requirements.txt"
...
install_trellis2_wheels() {
    ...
    for whl in "${wheels[@]}"; do
        ...
        if python3.12 -c "import $pkg_name" 2>/dev/null; then
            info "  Already installed: $pkg_name"
        else
            log "  Installing: $whl"
            python3.12 -m pip install "$whl_path" --no-deps
        fi
    done
...
install_trellis2_requirements() {
    ...
    if python3.12 -m pip install -r "$req_file"; then
    ...
    while IFS= read -r pkg || [ -n "$pkg" ]; do
        ...
        [[ -z "$pkg" || "$pkg" == \#* ]] && continue
        python3.12 -m pip install "$pkg" || warn "  Could not install: $pkg (skipping)"
    done < "$req_file"
...
main() {
    ...
    echo -e "${GREEN}║  Python 3.12  |  PyTorch 2.9.1  |  CUDA 12.8        ║${NC}"
